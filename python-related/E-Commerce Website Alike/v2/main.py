...
class UserDetails:
    ...
    def calculate_purchasing_cost(self):
        ...
        if not self.items_in_cart:
            ...
            item_quantities = {
                item.items_id: item.quantity for item in self.items_in_cart
            }
        ...
        return [
            {
                "price_data": {
                    ...
                    "unit_amount": int(
                        round(shop_items[cart_item.items_id].price * 100)
                    ),
                },
                "quantity": cart_item.quantity,
            }
            for cart_item in cart_items
            if cart_item.items_id in shop_items
        ]
    ...
    def get_total_cart_items_count(self):
        return (
            ...
            .filter(Cart.user_id == self.user_id)
            .scalar()
        )
...
def pass_details():
    ...
    if current_user.is_authenticated:
        counts = (user_info.user_unique_items_count, user_info.user_cart_items_count)
        return counts
...
def register_page():
    ...
    if register_form.validate_on_submit():
        ...
        if User.query.filter_by(
            ...
            flash(
                message="You've already signed up with this email, consider logging in..."
            )
        ...
        db.session.commit()
        login_user(new_user)  # logs in user immediately...
...
def login_page():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            flash(
                message=f"This email({email}) does not exist. Please check and try again!"
            )

        elif not check_password_hash(user.password, password):
            flash(
                message="Oh no! You have mistaken the password. Did you forget your precious password?"
            )
    ...
    return render_template(
        "login.html", form=login_form, this_user=current_user, details=pass_details()
    )
...
def add_to_cart(item_id):
    ...
    # capturing incoming value through AJAX & converting to integer...
    ...
    if item_already_in_cart(item_id):
        item_to_update = Cart.query.filter_by(
            items_id=item_id, user_id=current_user.id
        ).first()

        if item_to_update:
            if quantity > buying_item.stock:
                return jsonify(
                    {"status": "fail", "error": "Not enough stock available"}
                )
            ...
            db.session.commit()
            return jsonify(
                {"status": "success", "item_id": item_id, "quantity_added": quantity}
            )

    else:
        # noinspection PyArgumentList
        new_cart_record = Cart(
            items_id=item_id, quantity=quantity, user_id=current_user.id
        )
    ...
    try:
        if current_user.is_authenticated:
            ...
            return render_template(
                "shop-cart.html",
                this_user=current_user,
                eshop_items=shop_items,
                cart_items=cart_items,
                final_prices=checkout_prices,
                details=pass_details(),
            )
        else:
            return redirect(url_for("login_page"))
...
def update_cart():
    ...
    for update in updates:
        ...
        selected_product_in_cart = Cart.query.filter_by(
            items_id=item_id, user_id=current_user.id
        ).first()
        ...
        def set_values_for(mode):
            ...
            if mode == "update":
                ...
                selected_product_in_cart.quantity = (
                    current_quantity + increase - decrease
                )
...
def success():
    return render_template(
        "success.html", this_user=current_user, details=pass_details()
    )
...
def cancel():
    return render_template(
        "cancel.html", this_user=current_user, details=pass_details()
    )
